...
const handleUpdate=(event)=>{
  ...
  const { data: departments, isLoading: isDepartmentLoading, isError: isDepartmentError, isSuccess: isDepartmentSuccess, error: departmentError } = useQuery(
    ['departments'],
    getDepartment
  );
  ...
  if (isInterviewerLoading || isDepartmentLoading || isPositionLoading) return 'Loading...';
  ...
  if (departmentError) return `An error has occurred: ${departmentError.message}`;
    ...
    <>
      <form onSubmit={handleUpdate}>
        <div>
          ...
          />
          <select name="department" value={interviewer.department_id} 
          onChange={(e)=>setInterviewer({...interviewer,department_id:e.target.value})}>           
            <option value="">Select Department</option>
            {
              departments.map((department) => (
                <option key={department.id} value={department.id} selected={department.id === interviewer.department_id}>
                  {department.name}
                </option>
              ))
            }
          </select>
          <select name="position" value={interviewer.position_id}
          onChange={(e)=>setInterviewer({...interviewer,position_id:e.target.value})}>           
            <option value="">Select Position</option>
            {
              positions.map((position) => (
                <option key={position.id} value={position.id} selected={position.id === interviewer.position_id}>
                  {position.name}
                </option>
              ))
            }
